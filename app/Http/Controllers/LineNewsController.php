...
class LineNewsController extends Controller
{
    public function index($city = null)
    {
        $metaData = [
            'title' => 'Стрічка новин',
            'description' => 'Головні новини в прямому ефірі, розслідування, думки, фотографії та відео журналістів з багатьох країн світу. Ми висвітлюємо українські та міжнародні новини, серед яких: політика, бізнес, технології, наука, здоров\'я, мистецтво, спорт, та інші події з багатьох куточків світу.',
        ...
        $LOCAL_NEWS = "http://localhost:4444/api/newsattr?limit=100&feedMatch={$encodedFeed}";
        $SERVER_NEWS = "https://sside.daycom.com.ua/api/newsattr?limit=100&feedMatch={$encodedFeed}";
        ...
        if ($response) {
            ...
            if ($data !== null) {
                ...
                // return view('line_news', ['metaData' => $metaData, 'data' => $data, 'dataBanners' => $dataBanners]);
                if ($city === null) {
                    return view('line_news', ['metaData' => $metaData, 'data' => $data, 'dataBanners' => $dataBanners]);
                } else {
                    $currentCityData = $cityData[$city];
                    return view('city_line_news', ['metaData' => $metaData, 'data' => $data, 'cityData' => $currentCityData, 'city' => $currentCityData, 'dataBanners' => $dataBanners]);
                }
            } else {
